...
#read in drugs data from the ONS
...
#merge into alcohol
data <- merge(dataalc, datadrg, by.x="Area Code", by.y="id", all.x=TRUE)
...
#Read in Scottish data - replace "..." with location of Scottish .xlsx file

dataalc <- read_excel("...", sheet="Alcohol-specific deaths",
                      range="A9:D40", col_names=FALSE)

colnames(dataalc) <- c("LA", "alcrate", "alcrate_lower_ci", "alcrate_upper_ci")

datadrg <- read_excel("...", sheet="Drug-related deaths",
                      range="A9:D40", col_names=FALSE)
...
#Replace "..." with location of shapefile
polygons <- readOGR("...")
...
map <- ggplot(data)+
  ...
  theme_classic()+
  scale_fill_identity()+
  labs(title="Regional patterns in deaths from alcohol and drugs in England & Scotland",
       subtitle="Comparative rates of alcohol-specific deaths and deaths from drug misuse by Local Authority 2016-18",
       caption="Data from Public Health England, Office for National Statistics & National Records of Scotland\nPlot by @VictimOfMaths")+
  #Highland S12000017
  ...
  #Walsall E08000030
  annotate("text", x=230000, y=240000, label="Blue areas mean\nhigh rates of alcohol and \nlow rates of drug deaths", size=3)+
  #Hampshire E10000014
  ...
  geom_curve(aes(x=293000, y=235000, xend=405000, yend=300000), curvature=0.1)+
...
ggdraw()+
  ...
  draw_plot(key, 0.03,0.15,0.29,0.41)
dev.off()
...
ggplot(data, aes(x=alcrate, y=drgrate, colour=Country))+
  ...
  scale_colour_manual(values=c("#CF142B", "#0065BF"))+
  ...
  labs(title="Deaths from alcohol and drugs by Local Authority", 
       ...
       caption="Data from PHE, ONS & NRS | Plot by @VictimOfMaths")
dev.off()
...
ggplot(data, aes(x=fct_reorder(as.factor(LA), alcrate), y=alcrate, ymin=alcrate_lower_ci, 
  ...
  geom_crossbar()+
  theme_classic()+
  scale_fill_manual(values=c("#CF142B", "#0065BF"))+
  ...
  labs(title="Local Authority variation in alcohol-specific deaths", 
       subtitle="Mean annual death rates 2016-18 with 95% Confidence Intervals",
       caption="Data from PHE, ONS & NRS | Plot by @VictimOfMaths")+
